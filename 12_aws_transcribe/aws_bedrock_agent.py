...
class AWSBedRockAgent(AWSBase):
    ...
    3. For each question, provide:
        - startTime, endTime (from the commentary segment)
        - answerTime: the **exact time in seconds when the answer can be known**, based on the commentary
        - question text
        - answerType: "yes/no", "multiple_choice", or "count"
        - answer: Yes/No, exact number, or choice
        - explanation: reasoning **based strictly on the commentary segment**
    4. Ensure:
        
        - No repeated or reworded questions
        - Will the match end with more than 2.5 total goals? These kinds of questions are filtered out as goals can't be in fraction(not realistic question).
        - Questions are realistic and contextually relevant
        - JSON output is a flat array of objects
    ...
    [
      {{
        ...
        "answer": "{teamA}",
        "answerTime": 540.0, (here answerTime is once match is over)
        "explanation": "{teamA} is dominating possession and creating multiple clear chances, suggesting they are likely to win if this trend continues."
      }},
      {{
        "startTime": 15.0,
        "endTime": 25.0,
        ...
        "answerType": "yes/no",
        "answer": "No",
        "answerTime": 20.0,(startTime+5mins)
        "explanation": "No significant fouls or defensive errors are observed in this segment, so a penalty is unlikely within the next 5 minutes."
      }},
      {{
        "startTime": 30.0,
        "endTime": 42.0,
        "question": "Will Ronaldo score in this sequence?",
        "answerType": "yes/no",
        "answer": "Yes",
        "answerTime": 35.5,
        "explanation": "Ronaldo is in a prime scoring position and the commentary mentions he is taking a shot on goal in this segment."
      }},
      {{
        "startTime": 50.0,
        "endTime": 60.0,
        "question": "Will Spain maintain possession for the next 30 seconds?",
        "answerType": "yes/no",
        "answer": "Yes",
        "answerTime": 80.0,
        "explanation": "Spain has controlled the ball consistently and the commentary notes their passing sequence is uninterrupted, indicating they will retain possession."
      }},
      
      {{
        "startTime": 100.0,
        "endTime": 110.0,
        "question": "How many goals Ronaldo has scored in in his career?",
        "answerType": "count",
        "answer": 82,
        "answerTime": 105,(startTime+5s, answerTime for these questions which are globally known should be just after startTime)
        "explanation": "Ronaldo has scored total 82 goals in his career"
      }},
      {{
      "startTime": 0.0,
      "endTime": 11.84,
      "question": "Will Portugal use a 4-4-2 formation in this match?",
      "answerType": "yes/no",
      "answer": "Yes",
      "answerTime": 300,
      "explanation": "The commentary mentions Portugal regularly used a 4-4-2 formation in qualifying, suggesting they are likely to continue with this tactical setup in the match.",
      }},
    ]
    ...
    @staticmethod
    def filter_questions_by_gap(questions, min_gap=30):
        """
        Keep questions spaced at least `min_gap` seconds apart based on startTime.
        """
        final_questions = []
        last_time = -min_gap  # ensures first question is always included
        for q in questions:
            if q['startTime'] - last_time >= min_gap:
                final_questions.append(q)
                last_time = q['startTime']
        return final_questions

    @staticmethod
    def split_segments_for_questions(audio_segments, max_segment_duration=30):
        """
        Split large audio segments into smaller chunks of `max_segment_duration` seconds.
        This ensures questions are generated roughly every 30s or 40s.
        """
        new_segments = []
        for seg in audio_segments:
            start, end, text = seg['start_time'], seg['end_time'], seg['transcript']
            duration = float(end) - float(start)
            if duration <= max_segment_duration:
                new_segments.append(seg)
            else:
                # Split into smaller segments
                n_splits = int(duration // max_segment_duration) + 1
                step = duration / n_splits
                for i in range(n_splits):
                    sub_start = start + i * step
                    sub_end = min(start + (i + 1) * step, end)
                    new_segments.append({
                        'start_time': sub_start,
                        'end_time': sub_end,
                        'transcript': text  # keep same transcript, optionally truncate
                    })
        return new_segments

    def generate_questions_for_transcript(self, transcript_json, batch_size=5, max_workers=10, max_questions=100, min_gap=30):
        ...
        # Split segments to ensure ~30-40s question frequency
        audio_segments = self.split_segments_for_questions(audio_segments, max_segment_duration=min_gap)

        all_batches = [audio_segments[i:i + batch_size] for i in range(0, len(audio_segments), batch_size)]
        all_questions = []

        # Call Bedrock in parallel
        ...
        # 3. Ensure min_gap between questions
        all_questions = self.filter_questions_by_gap(all_questions, min_gap=min_gap)

        # 4. Fallback for answer_time if missing
        for q in all_questions:
            if 'answerTime' not in q:
                q['answerTime'] = q.get('endTime', q['startTime'])
